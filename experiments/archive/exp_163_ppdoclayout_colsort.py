...
def sort_columns(regions: list[dict[str, tp.Any]]) -> list[dict[str, tp.Any]]:
    columns: list[dict[str, tp.Any]] = []
    for region in sorted(regions, key=lambda r: r["bbox"][0]):
        x1, _, x2, _ = region["bbox"]
        placed = False
        for col in columns:
            overlap = min(x2, col["x2"]) - max(x1, col["x1"])
            if overlap / max(1, min(x2 - x1, col["x2"] - col["x1"])) >= 0.5:
                col["regions"].append(region)
                col["x1"] = min(col["x1"], x1)
                col["x2"] = max(col["x2"], x2)
                placed = True
                break
        if not placed:
            columns.append({"x1": x1, "x2": x2, "regions": [region]})
    columns.sort(key=lambda c: c["x1"])
    ordered: list[dict[str, tp.Any]] = []
    for col_idx, col in enumerate(columns):
        col["regions"].sort(key=lambda r: r["bbox"][1])
        for pos, region in enumerate(col["regions"]):
            region["col"] = col_idx
            region["col_last"] = pos == len(col["regions"]) - 1
            region["col_first"] = pos == 0
        ordered.extend(col["regions"])
    return ordered
...
def assemble(date: str, page_count: int, texts: list[str], regions: list[dict[str, tp.Any]]) -> dict[str, tp.Any]:
    ...
    for text, region in zip(texts, regions):
        ...
        if region["class"] == "title":
            ...
            fresh_title = current is not None and current["headline"] is not None and not current["paragraphs"]
            continues = False
            if prev_region is not None and current is not None:
                page_h = page_heights.get(region["page"], 4000)
                same_page = prev_region["page"] == region["page"]
                if same_page and prev_region.get("col") == region.get("col"):
                    continues = 0 <= region["bbox"][1] - prev_region["bbox"][3] <= 0.04 * page_h
                elif prev_region.get("col_last") and region.get("col_first"):
                    continues = True
            if not (fresh_title or continues):
                if current is not None:
                    articles.append(current)
...
def run_date(date: str) -> tuple[float, int]:
    ...
    regions_per_page = [sort_columns(build_regions(boxes, i + 1)) for i, boxes in enumerate(layout)]
