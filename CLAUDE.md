...
- **Corpus-scale budget** (5× raise 2026-07-22): full-corpus OCR (1880–1959, 172,600 pages) on 2×3090 → **target 103.5, hard cap 250.0 sec/page**. Budget is measured by the caller (`scripts/time_experiment.sh`), not the experiment; >250.0 is a research artifact, not production. `BUDGET_CAP` in scripts/progress_server.py is the single source of truth. The goal is stated in `GOAL.md`; operating manual in `eval/autoresearch/program.md`.
- **One script → one run → one result**: every experiment is a self-contained script `experiments/<name>.py <date...>` writing `eval/predictions/<name>_<date>.json` (contract in `experiments/README.md`). Experiments never import/run the eval; scoring is external via `scripts/research.py eval`. The `configs/ocr` + `run_real_ocr.py` harness serves the oracle-reference ensembles only.
