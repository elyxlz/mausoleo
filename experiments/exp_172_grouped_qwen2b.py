...
def ocr_crops(crops: list[Image.Image]) -> tuple[list[str], float]:
...
def run_date(date: str) -> tuple[dict, int, float]:
    ...
    if flat:
        starts[0] = 1
    return {"date": date, "source": "exp_172_grouped_qwen2b", "articles": _group(flat, texts, starts)}, len(pages), ocr_s
...
def main() -> None:
    ...
    for date in (sys.argv[1:] or list(DATES)):
        ...
        (PRED_DIR / f"exp_172_grouped_qwen2b_{date}.json").write_text(json.dumps(pred, ensure_ascii=False))
        total_pages += npages
        total_ocr_s += ocr_s
