...
def probe(date: str) -> dict[str, float]:
    ...
    scrambled = {
        **gt,
        "articles": [
            {
                **a,
                "headline": (_scramble(a["headline"], rng) if a.get("headline") else None),
                "paragraphs": [{"text": _scramble(p["text"], rng)} for p in a["paragraphs"]],
            }
    ...
    page_blob = {
        **gt,
        "articles": [{"headline": None, "paragraphs": [{"text": " ".join(txts)}], "page_span": [pg]} for pg, txts in sorted(by_page.items())],
    }
...
def main() -> None:
    ...
    for date in DATES:
        ...
        if r["clean"] < 0.95:
            print("  FAIL clean should be ~1.0")
            ok = False
        for k, lim in limits.items():
            if r[k] > lim:
                print(f"  FAIL {k}={r[k]:.3f} > {lim}")
                ok = False
