...
def assemble(date: str, page_count: int, texts: list[str], regions: list[dict[str, tp.Any]]) -> dict[str, tp.Any]:
    ...
    row = {
        ...
        "page_texts": json.dumps(texts),
        "layout_json": json.dumps(regions),
    }
    merged = merge_pages(row, config=MergePages(title_class_headlines=True, squeeze_char_runs=True))
    articles = json.loads(merged["result_json"])["articles"]
