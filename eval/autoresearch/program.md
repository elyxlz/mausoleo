...
## Resource Budget (3× increase 2026-07-21, per Elio — was 6.9–13.9)
Goal: OCR the ENTIRE corpus (1880 → 1959, **172,600 pages** measured on endeavour ≈ 29K issues; 498,252 pages exist 1880–2000) on ripperred. Budget raised 3× → target ~3 weeks, ~6 weeks absolute max on 2×3090.

| Target | sec/page | sec/issue (avg 7pp) | wall on 2×3090 (172.6K pp) |
|---|---|---|---|
| target | **20.7** | ~145 | ~21 days |
| hard cap | **50.0** | ~350 | ~50 days (~7 wk) |

- **DISQUALIFICATION LINE = 50.0 sec/page** (caller-measured wall time). Raised from 41.7 to absorb model-load + one-time overhead now included in the caller measure (~1 sec/page real, generous buffer for larger models). `BUDGET_CAP` in `scripts/progress_server.py` + `scripts/seed_progress.py` is the single source of truth; the dashboard recomputes `budget_ok` from `gpu_s_per_page` (= caller sec/page) vs the cap.
- Budget accounting is **steady-state throughput** (continuous batching, model loaded once), not cold-start single-issue latency. Measure with ≥2 issues back-to-back, exclude model load.
- Every experiment/run MUST record measured GPU-s/page; a config that beats the score but exceeds 50.0 GPU-s/page steady-state is a research artifact, not a production candidate.
- The 3× raise does NOT unlock the Qwen3-VL-8B routes (~74–286 GPU-s/page, still 2–7× over the 50.0 cap); it opens headroom for stronger mid-size models (up to ~7B territory that lands ≤50.0 steady-state).
...
## Experiment Contract & Adversarial Review (MANDATORY — per Elio 2026-07-21)
- **Experiments NEVER import the eval.** An `experiments/<name>.py` may not `import mausoleo.eval` / call `evaluate_issue` / read `ground_truth.json` to score or to select a hyperparameter. It produces predictions from inference-time information only, with fixed a-priori choices (thresholds, primary source). Tuning a knob against the eval score is overfitting and will not generalize to unlabeled issues. **Evaluation happens ONLY outside**, via `scripts/research.py eval`. (Training a model on GT labels and applying it — e.g. the boundary grouper — is fine; scoring/selecting against the eval is not.)
- **Every experiment gets an adversarial review** before its number is trusted, checking for matcher-gaming: (1) overgeneration — preds ≫ GT count; (2) giant blobs / cer>1 matches; (3) holdout-half regression (`research.py holdout`); (4) GT-free probe degradation (`research.py probe`: lexicon validity, repetition, tiny/huge share) vs parents/baseline. State the verdict (real vs gamed) and any caveats in the log entry. A high score with degraded probes or spam is rejected, not logged as a win.
- **Independence + cold start (per Elio 2026-07-21).** Every experiment must be fully self-contained and reproducible from scratch: it may NOT be sped up by, or depend on, artifacts produced by other/previous experiment runs — no reading cached region dumps, another experiment's prediction files, or intermediate OCR to skip its own compute. Running it on a fresh machine must reproduce the same predictions and the same `gpu_s_per_page`. (Caching itself is fine — within-run vllm prefix/KV cache, or a model trained offline on the fixed GT dataset — as long as every cache is COLD at the start of the experiment and nothing warm is carried over from a prior run to look faster.) An experiment that leans on a prior run's cache to look cheap or fast is disqualified until re-run independently. Template: exp_172 (fresh PP-DocLayout + fresh OCR, self-contained).
- **Budget = total wall seconds of the run, measured by the CALLER (simpler, per Elio 2026-07-21).** The experiment script does NOT measure or report its own budget — it just writes predictions. The caller (`scripts/time_experiment.sh experiments/<name>.py`) times the whole invocation start→finish over the 6 eval issues (model load + PP-DocLayout + OCR + grouping + IO all included) and reports `sec_per_page = wall_seconds / 42 pages`. That number is `gpu_s_per_page` in the log; cap = 50.0. This is deliberately a conservative UPPER bound: it includes one-time model load and per-issue layout-subprocess reloads that amortize away at corpus scale, so passing here means passing on the corpus. Near-cap candidates can be re-measured more carefully (persistent layout, larger batch) before disqualifying. Note the earlier budget-compliant entries (exp_157/160/167/171/172/173) used an OCR-only in-script measure; new entries use caller wall-time — both sit far under the 50.0 cap, so the unit change doesn't flip any verdict. Qwen3-VL-8B *column* route (~136) stays DQ; the 8B *per-region* route is being measured cold (exp_174) since the 136 was the wrong route.
