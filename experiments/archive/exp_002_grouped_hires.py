...
def run_date(date: str) -> dict:
    ...
    if flat:
        starts[0] = 1
    return {"date": date, "source": "exp_002_grouped_hires", "articles": _group(flat, texts, starts)}


def main() -> None:
    for date in sys.argv[1:] or list(DATES):
        pred = run_date(date)
        (PRED_DIR / f"exp_002_grouped_hires_{date}.json").write_text(json.dumps(pred, ensure_ascii=False))
