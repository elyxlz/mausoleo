...
- **Budget must be measured COLD, no cache (per Elio 2026-07-21).** `gpu_s_per_page` is measured on a from-scratch run: no vllm prefix caching (`enable_prefix_caching=False`), no reusing cached region dumps / prior predictions / intermediate OCR as if free. The number must reflect what the corpus run actually pays per page (model load amortized over the issue, everything else recomputed). An experiment that reads a cache to look cheap is disqualified until re-measured cold. Over-budget (>13.9 GPU-s/page cold) → does not count.
