...
- exp_168 confirms text-quality lever but doesn't beat exp_045 and is over budget. **Per-issue oracle-select exp_045 ⊕ exp_168 = 0.5250** (1885/1895/1935→168, 1910/1925/1952→045) — well above exp_045 (0.4615), 89% of the way to the oracle ceiling (0.5941). The two routes are genuinely complementary → a real article-level MERGE of the two prediction sets is the top next experiment (free, no GPU; borrow the ensemble merge operator). Then: (b) exp_045-quality text under budget (smaller VLM / PP-DocLayout reading-order → single structured pass) + trained grouper; (c) 1952 dense-classifieds is the common weak point across every route — targeted look.
- Dashboard front-end rebuilt (Fable): interactive SVG graph (hover tooltips, monotonic record staircase, oracle ceiling lines), sleek dark theme, Record-setting/All filter. Live-render verified on :8078 (12 attempts, filter 1↔12, no JS errors).
