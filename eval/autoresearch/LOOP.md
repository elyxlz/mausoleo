...
2. **exp_012 LoRA fine-tune PaddleOCR-VL-1.6 — TRAINING NOW** (chained job bgmpl4jc1: train no-1935 fold → merge → infer 1935 → eval vs base 0.430). Fold: 261 train/29 val article-crop→GT-text pairs, strict LOIO (1935 held out). ms-swift LoRA r8 all-linear, vision frozen. Base model is 88% token-accurate on these pairs (headroom). If fine-tuned 1935 beats base wCER/score meaningfully → full 6-fold LOIO + synthetic period-Italian augmentation; else reconsider (distill oracle, ByT5 corrector). Pipeline: experiments/ft_prepare_data.py, ft_train_lora.sh, exp_012_paddleft.py (transformers pinned 5.5.4; ms-swift 4.4.2; USE_HF=1).
