...
def _engine() -> tuple[tp.Any, tp.Any]:
    if "llm" not in _ENGINE:
        ...
        _ENGINE["llm"] = LLM(model=MODEL, trust_remote_code=True, gpu_memory_utilization=0.90,
                             ...
                             enable_prefix_caching=False, max_num_seqs=32, seed=0)
...
def ocr_crops(crops: list[Image.Image]) -> list[str]:
    ...
    for img in crops:
        ...
        messages = [{"role": "user", "content": [{"type": "image", "image": img},
                     {"type": "text", "text": "Transcribe all the text in this image exactly, in reading order. Output only the transcription."}]}]
...
def run_date(date: str) -> dict:
    ...
    return {"date": date, "source": "exp_007_article_qwen8b", "articles": articles}


def main() -> None:
    for date in (sys.argv[1:] or list(DATES)):
        pred = run_date(date)
        (PRED_DIR / f"exp_007_article_qwen8b_{date}.json").write_text(json.dumps(pred, ensure_ascii=False))
