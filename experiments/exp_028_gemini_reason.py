...
CACHE_DIR = pl.Path("eval/cache/transcribe")
...
REGROUP_PROMPT = (
    ...
    "MOST UNITS ARE ALREADY CORRECT AND MUST BE LEFT ALONE. Merge only where there is positive "
    "evidence on the page. When in doubt, do NOT merge.\n\n"
    "Merge in exactly these three situations, and no others:\n"
    "1. A story broken at a column or page boundary, where the text stops MID-SENTENCE and the next "
    "unit resumes it. The mid-sentence break is required evidence — a unit that ends with a full stop "
    "and is followed by a new topic is a SEPARATE article.\n"
    "2. A classified RUBRIC heading and the adverts printed beneath it: 'AUTOMOBILI BICICL. (L. 90)', "
    "'VILLINI CASE TERRENI', 'MATRIMONIALI'. The heading unit plus every advert until the next heading "
    "become ONE article. A rubric heading is short, capitalised, and often carries a price like "
    "'(L. 90)'.\n"
    "3. Nothing else. In particular: distinct rubrics stay separate from each other; two stories on "
    "the same topic stay separate; a headline and the story beneath it are ALREADY one unit.\n\n"
    "NEVER merge:\n"
    "- adverts or notices printed on their own, not under a rubric heading — each is its OWN article, "
    "even if several sit next to each other and look similar;\n"
    "- units that merely share a subject, a page, or a column;\n"
    "- units separated by a completed sentence and a new headline.\n"
    "Rubric runs are long — often dozens of consecutive units — so once inside one, keep going until "
    "the next rubric heading.\n"
    "Cross-page: a story or rubric running past the bottom of a page continues on the next, so a group "
    "may span pages — but ONLY with the mid-sentence evidence in rule 1.\n\n"
    "Give each article the printed headline, or null.\n"
    "CONTIGUITY RULE: a group must be a RUN OF CONSECUTIVE unit numbers, e.g. [12, 13, 14]. Articles "
    "and rubrics are printed continuously in reading order, so their pieces are always adjacent. "
    "Never group non-adjacent numbers such as [3, 47, 89] — if two distant units look related, they "
...
def _cache_path(image: bytes) -> pl.Path:
    digest = hashlib.sha1(image).hexdigest()[:16]
    return CACHE_DIR / f"{MODEL}_ultra_{digest}.txt"


def _transcribe_cached(client: tp.Any, image: bytes) -> str:
    path = _cache_path(image)
    if path.exists():
        return path.read_text()
    text = _transcribe_page(client, image)
...
def _regroup(client: tp.Any, articles: list[dict[str, tp.Any]]) -> list[dict[str, tp.Any]]:
    ...
    for group in groups if isinstance(groups, list) else []:
        ...
        if len(runs) > 1:
            ...
            dropped = sum(len(r) for r in runs) - len(longest)
            non_contiguous[0] += dropped
            idx = longest
        used.update(idx)
    ...
    if non_contiguous[0]:
        print(f"  dropped {non_contiguous[0]} non-contiguous unit(s) from groups", flush=True)
...
def _call_text(client: tp.Any, prompt: str) -> str:
    ...
    config = types.GenerateContentConfig(
        ...
        max_output_tokens=MAX_OUTPUT_TOKENS,
        response_mime_type="application/json",
        thinking_config=types.ThinkingConfig(thinking_budget=REGROUP_THINKING),
    )
    for attempt in range(MAX_TRIES):
        try:
...
def _retry_weak_pages(client: tp.Any, pages: list[bytes], texts: list[str]) -> list[str]:
    for _ in range(WEAK_PAGE_RETRIES):
        ...
        for i, text in zip(weak, fresh):
            if _article_count(text) > _page_chars(texts[i]) and _article_count(text) > 0:
                CACHE_DIR.mkdir(parents=True, exist_ok=True)
                _cache_path(pages[i]).write_text(text)
        improved = False
        for i, text in zip(weak, fresh):
...
def _run_issue(client: tp.Any, date: str) -> Issue:
    ...
    with cf.ThreadPoolExecutor(max_workers=CONCURRENCY) as pool:
        raw_texts = list(pool.map(lambda page: _transcribe_cached(client, page), pages))
...
def main() -> None:
    ...
    for date in sys.argv[1:] or DATES:
        ...
        out = PRED_DIR / f"exp_028_gemini_reason_{date}.json"
