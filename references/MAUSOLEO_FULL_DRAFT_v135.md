...
A digitised newspaper corpus normally allows a historian to retrieve articles by keyword, with date as a facet on the side. For the July 1943 *Il Messaggero* corpus this dissertation works with, that template breaks down on the questions the corpus most invites. The morning paper for 26 July 1943 was not printed: the Grand Council had deposed Mussolini overnight and an editorial line could not be drawn in time, so a flat article index returns nothing for the date. Questions about the regime-change days of 25 to 27 July return tens of articles the reader assembles by hand; questions about the war-and-domestic balance across the month return hundreds. This dissertation builds Mausoleo, a system that stores the corpus as a calendar-shaped tree of recursively summarised nodes over which a researcher agent navigates through time and structure.

A century of cognitive-science work has accumulated evidence that memory organises temporal information at multiple resolutions, and researchers reading time-stamped material shift between date-bound items and larger schemas built up over weeks. A flat ranked-article interface forces that integration onto the reader. Chapter four runs three case studies on whether exposing temporal structure in the index changes how the work goes: what the paper said on the absent 26 July, how it covered the regime change of 25 to 27 July, and how the balance of war and domestic-politics coverage moved across the month. Across eighteen scored trials the system averaged 11.3 tool calls against the keyword baseline's 28.3 and was preferred by both judges on a three-dimension rubric in every case. The calendar-shaped index holds a day node for 26 July with empty leaves and a summary that records the absence as evidence, a slot for documented silence that a flat article index cannot provide.
...
I thank Dr Yi Gong for supervising. Her comments on what the system can and cannot warrant shaped the chapters that follow.
...
Tolman (1948) ran rats through mazes and concluded they were building something like a map. Eichenbaum (2017), reviewing decades of single-cell hippocampal recording, argued that the same circuit also indexes time and conceptual relation. Whittington et al. (2020) made the underlying claim formal in the Tolman-Eichenbaum-Machine, where one factorisation of structure and content does spatial navigation, sequence prediction and analogical generalisation in the same model. So when a historian reads a month of newspapers at several time-scales, the cognitive resource being taxed is the same one the hippocampal-entorhinal system has been running on spatial problems for as long as it has existed. An interface that lays the multi-resolution structure out externally relieves that load; a flat keyword search hands it back to the reader, and the reader must then juggle day, week and month resolutions in working memory while still searching.

This dissertation builds and tests an interface that does hold them. Mausoleo stores the *Il Messaggero* July 1943 corpus as a calendar-shaped tree. Leaves are paragraphs from the actual articles; the level above is article summaries; above that day, then week, then a month root that sits at the top. Where there is no issue (26 July) there is still a node, kept empty by design. The week node spanning 25 July carries an integrated summary of the regime-change days, and the month root records the war-and-domestic shape against which the per-week balance can be read. The researcher-side agent walks the tree and can also issue keyword or embedding queries against the same corpus. Three case studies on the July 1943 material, evaluated against a keyword baseline over identical article transcriptions, ask whether this multi-resolution interface beats the flat one on those three kinds of question.
...
### Existing digitised newspaper archives, and what they let you ask
...
### Hierarchical retrieval: what it does that flat retrieval cannot
...
Bartlett's (1932) work, including the *War of the Ghosts* studies, established that recall preserves the gist as a *schema*. The detail attrits; the compressed template does not. Craik and Lockhart (1972) recast the durability of memory as a question about depth of processing, where deeper semantic encoding leaves a more durable trace. Subsequent psychology has separated episodic memory, which is bound to time and place, from semantic memory, which has shed those bindings, with schemata sitting above the latter as the most compressed level. An archival corpus presents the same problem in external form. Articles read at the leaf are episodic; chapter four's per-week summaries are something more like semantic; the month-long shape is a schema. A reader who only has the leaves still has to do the compression, just internally and on every visit.
...
### From scanned pages to article transcriptions, in five passes
...
Construction proceeds bottom-up, one level at a time. A batch process summarises each article from its own paragraphs, and the same recursive procedure runs at coarser grain for the day, week, and month nodes. The procedure follows Wu et al. (2021), whose recursive book summarisation showed that fixed-branch bottom-up summarisation produces usable abstractions without ever blowing the context window. I kept the summariser prompt the same at every level. The wording asks for two to four hundred words, demands that named entities be folded into prose rather than listed, requires dates and quantities to be preserved verbatim, and requires the summary to be useful enough that an agent reading it could decide on its own whether to descend or not. Holding length constant has the side-effect that the higher levels are forced into much harsher thematic compression than the lower ones. A spot-check against ten July 1943 day summaries (excluding the absent 26 July) recovered forty-eight of fifty top named entities through accent-stripped substring match against the day's hand-cleaned transcription; the two misses were both `Papa Pio XII`, where the summariser inserted an editorial honorific the press itself never used, preferring `Pio XII` or `il Pontefice`. An information-loss trace on 1943-07-03 establishes the compression rate precisely: of thirty-six distinct named entities in the day's three longest articles, seven survive at day level, six at week, one (`Il Messaggero` itself) at month. What survives is not random. Acronyms for generic bureaucratic bodies disappear first, usually by the time the week summary is written; named individuals last longer (most of them are still recoverable from the week, though by the month-level summary almost all of them have been absorbed into descriptive paraphrase); place names fall in between. The practical consequence is that the month-level node is a reasonable place to look for the gist of what kind of month July 1943 was, but a bad place to look up who specifically appeared in the paper that week.
...
The reading loop is a ReAct loop in the sense of Yao et al. (2022), not single-shot retrieval-augmented generation in the sense of Lewis et al. (2020). It always starts at the month root. From there, the agent looks at the summary it has just read, decides whether the question is answered or not yet, picks one of the children to descend into, or issues a keyword or embedding query if going down the calendar is not the right move. Date-bearing questions tend to fall out of the traversal naturally; for the others the search endpoints exist as a lateral move. The interface itself is deliberately thin. There are no helper steps inside the API, no stored conversation state, and no privileged retrieval policy beyond the agent's own choice of next step.
...
In Mausoleo every date in the month range gets a day node, whether or not there was an issue printed that day. The 26 July node is empty in the leaf sense (no article children) but it still carries a day-level summary saying so, and the agent can read that summary directly. A keyword-only retriever sitting on top of the same article corpus has no comparable surface to read; the absence of articles for that date is, from its point of view, indistinguishable from a query that simply did not match anything. To get equivalent behaviour out of the keyword retriever I would have had to bolt on metadata from outside the index — for instance a manually-curated list of known-missing dates, or an extra instruction inside the agent's system prompt telling it to check for absences before giving up — and one of those workarounds is exactly the kind of fragile side-channel the calendar-shaped index avoids by treating the gap as something the data model already knows about.
...
### Cases 2 and 3: two shorter contrasts
...
Doucet, A., Gasteiner, M., Granroth-Wilding, M., Kaiser, M., Kaukonen, M., Labahn, R., Moreux, J.-P., Muehlberger, G., Pfanzelter, E., Therenty, M.-E., Toivonen, H. and Tolonen, M. (2020) 'NewsEye: a digital investigator for historical newspapers', in *Proceedings of Digital Humanities 2020*. Ottawa: ADHO.
...
Eichenbaum, H. (2017) 'The role of the hippocampus in navigation is memory', *Journal of Neurophysiology*, 117(4), pp. 1785–1796. doi:10.1152/jn.00005.2017.
...
Murugaraj, M., Lamsiyah, S., Düring, M. and Theobald, M. (2025) 'Topic-RAG for historical newspapers: enhancing information retrieval in humanities research through topic-based retrieval-augmented generation', *Computational Humanities Research*, 1(e15), pp. 1–24. doi:10.1017/chr.2025.10018.
