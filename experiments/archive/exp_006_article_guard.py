...
def run_date(date: str) -> dict:
    ...
    for gi, grp in enumerate(groups):
        ...
        la, lr = len(art_text), len(region_concat)
        text = art_text if (lr == 0 or 0.5 * lr <= la <= 1.8 * lr) else region_concat
        if not text.strip():
            text = region_concat or art_text
        if not text.strip():
            continue
    ...
    return {"date": date, "source": "exp_006_article_guard", "articles": articles}


def main() -> None:
    for date in sys.argv[1:] or list(DATES):
        pred = run_date(date)
        (PRED_DIR / f"exp_006_article_guard_{date}.json").write_text(json.dumps(pred, ensure_ascii=False))
