...
from mausoleo.paths import BUDGET_CAP, EVAL_DATES, GT_DIR, LOG_PATH, PRED_DIR
...
def find_prediction_path(config: str, date: str) -> pl.Path | None:
    if not CONFIG_PATTERN.match(config) or date not in EVAL_DATES:
        return None
...
def find_image_path(date: str, page: str) -> pl.Path | None:
    if date not in EVAL_DATES or not page.isdigit():
        return None
    path = GT_DIR / date / f"{int(page)}.jpeg"
    return path if path.is_file() else None
