...
- **Budget = 3× (2026-07-21): target 20.7, hard cap 50.0 GPU-s/page steady-state.** `BUDGET_CAP` (progress_server.py + seed_progress.py) is the single source of truth; dashboard recomputes `budget_ok` from `gpu_s_per_page`. Corpus = 172,600 pages (1880–1959; 498,252 exist 1880–2000). Measure steady-state (model load amortized ~0/page; small runs are upper bounds).
...
## Board (budget-compliant only counts; ≤50.0 sec/page)
...
1. **exp_173 verdict** — if Qwen3-VL-4B ≤50.0 sec/page AND beats exp_167 0.3815 → new record. Log + adversarial-review.
