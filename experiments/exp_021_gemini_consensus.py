...
def _page_similarity(a: str, b: str) -> float:
    import difflib

    left = " ".join(p["text"] for art in page_articles(a) if isinstance(art, dict) for p in [{"text": str(art.get("text") or "")}])
    right = " ".join(p["text"] for art in page_articles(b) if isinstance(art, dict) for p in [{"text": str(art.get("text") or "")}])
    if not left or not right:
        return 0.0
    return difflib.SequenceMatcher(None, left[:6000], right[:6000]).ratio()


def _consensus(samples: list[str]) -> str:
    live = [s for s in samples if _article_count(s) > 0]
    if not live:
        return samples[0] if samples else ""
    if len(live) == 1:
        return live[0]
    scores = [sum(_page_similarity(a, b) for j, b in enumerate(live) if j != i) / (len(live) - 1) for i, a in enumerate(live)]
    return live[max(range(len(live)), key=lambda i: scores[i])]


def _transcribe_consensus(client: tp.Any, image: bytes) -> str:
    with cf.ThreadPoolExecutor(max_workers=SAMPLES) as pool:
        samples = list(pool.map(lambda _: _transcribe_page(client, image), range(SAMPLES)))
    return _consensus(samples)
...
def _run_issue(client: tp.Any, date: str) -> Issue:
    ...
    with cf.ThreadPoolExecutor(max_workers=max(1, CONCURRENCY // SAMPLES)) as pool:
        page_texts = [unwrap_page_json(text) for text in pool.map(lambda page: _transcribe_consensus(client, page), pages)]
    row: dict[str, tp.Any] = {
        "issue_id": date,
        "date": date,
...
def main() -> None:
    ...
    for date in sys.argv[1:] or DATES:
        ...
        out = PRED_DIR / f"exp_021_gemini_consensus_{date}.json"
