...
def _engine() -> tuple[tp.Any, tp.Any]:
    if "llm" not in _ENGINE:
        ...
        _ENGINE["llm"] = LLM(
            ...
            dtype="bfloat16",
            mm_processor_kwargs={"max_pixels": 1600000},
...
def run_date(date: str) -> dict:
    ...
    return {"date": date, "source": "exp_008_article_awq8b", "articles": articles}


def main() -> None:
    for date in sys.argv[1:] or list(DATES):
        pred = run_date(date)
        (PRED_DIR / f"exp_008_article_awq8b_{date}.json").write_text(json.dumps(pred, ensure_ascii=False))
