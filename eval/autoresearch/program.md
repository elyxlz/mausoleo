...
- **`gpu_s_per_page` = STEADY-STATE, not small-run overhead (per Elio 2026-07-21).** The corpus (172,600 pages, 1880–1959) runs the models resident and the GPU saturated, so the fair per-page cost is steady-state throughput, NOT a tiny 6-issue run's cost. One-time model load amortizes to ~0/page (loaded once for 172.6K pages) — exclude it. Per-issue overheads (subprocess model reloads, small under-saturated batches) inflate short runs: exp_172's 4-page issues measured 6.6–9.2 GPU-s/page but its 8–10-page issues ~4–5 — the per-page cost falls as batches grow. Measure at saturation (large continuous batch, persistent models) and report the steady-state figure; the 6-issue average is a conservative UPPER bound. GPU-s/page is GPU-count-agnostic (total GPU-seconds ÷ pages; more GPUs cut wall-clock, not GPU-seconds). Near-cap candidates (say 30–60 GPU-s/page by the quick estimate) must get a proper steady-state measurement before being disqualified. Over-budget (>41.7 GPU-s/page steady-state, the 3× cap) → does not count. Qwen3-VL-8B (~136) stays DQ even optimized (~3× over the raised cap).
