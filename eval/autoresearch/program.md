...
| Target | GPU-s/page | GPU-s/issue (avg 6pp) | wall on 2×3090 (172.6K pp) |
|---|---|---|---|
| target (3 wk) | **20.7** | ~124 | ~21 days |
| hard cap (6 wk) | **41.7** | ~250 | ~42 days |

- **DISQUALIFICATION LINE = 41.7 GPU-s/page** (steady-state). `BUDGET_CAP` in `scripts/progress_server.py` + `scripts/seed_progress.py` is the single source of truth; the dashboard recomputes `budget_ok` from `gpu_s_per_page` vs the cap.
...
- Every experiment/run MUST record measured GPU-s/page; a config that beats the score but exceeds 41.7 GPU-s/page steady-state is a research artifact, not a production candidate.
- The 3× raise does NOT unlock the Qwen3-VL-8B routes (~74–286 GPU-s/page, still 2–7× over the 41.7 cap); it opens headroom for stronger mid-size models (up to ~7B territory that lands ≤41.7 steady-state).
...
- **Budget = total wall seconds of the run, measured by the CALLER (simpler, per Elio 2026-07-21).** The experiment script does NOT measure or report its own budget — it just writes predictions. The caller (`scripts/time_experiment.sh experiments/<name>.py`) times the whole invocation start→finish over the 6 eval issues (model load + PP-DocLayout + OCR + grouping + IO all included) and reports `sec_per_page = wall_seconds / 42 pages`. That number is `gpu_s_per_page` in the log; cap = 41.7. This is deliberately a conservative UPPER bound: it includes one-time model load and per-issue layout-subprocess reloads that amortize away at corpus scale, so passing here means passing on the corpus. Near-cap candidates can be re-measured more carefully (persistent layout, larger batch) before disqualifying. Note the earlier budget-compliant entries (exp_157/160/167/171/172/173) used an OCR-only in-script measure; new entries use caller wall-time — both sit far under the 41.7 cap, so the unit change doesn't flip any verdict. Qwen3-VL-8B *column* route (~136) stays DQ; the 8B *per-region* route is being measured cold (exp_174) since the 136 was the wrong route.
