...
def ocr_columns(crops: list[Image.Image]) -> list[str]:
    ...
    outputs = llm.generate(prompts, SamplingParams(temperature=0.0, max_tokens=4096, repetition_penalty=1.05))
...
def run_date(date: str) -> dict[str, tp.Any]:
    ...
    return {"date": date, "source": "exp_003_column_structured", "articles": articles}


def main() -> None:
    for date in sys.argv[1:] or DATES:
        pred = run_date(date)
        (PRED_DIR / f"exp_003_column_structured_{date}.json").write_text(json.dumps(pred, ensure_ascii=False))
